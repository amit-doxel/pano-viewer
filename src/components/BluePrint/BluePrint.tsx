...
//const MARKER_SELECT_COLOR = '#454A4B';
...
const DEFAULT_SELECTION_TYPE = 'higlight';
...
type SelectionType = 'higlight' | 'pin';
...
// TODO:
// 1. Hook up onMarkerSelected
const BluePrint: React.FC<Props> = (props) => {

  // if selectedMarkerId changes don't want to rerender the whole canvas again
  ...
  useEffect(() => {
    ...
    //canvas$.setZoom(initialZoom || DEFAULT_ZOOM);
    ...
    fabric.Image.fromURL(
      ...
      function (img$: fabric.Image) {
        ...
        addWalkPathToCanvas(canvas$, markers, blueprintRenderOpts);
  ...
  }, [selectedMarker, bgImg$, canvas$]);

  //  useEffect(() => {
  //    // set zoom and pan around point
  //  }, [selectedMarkerId, zoom])
...
const BLUEPRINT_RENDER_OPTS_INIT_STATE = {
  ...
  topImgOffset: 0,
  leftImgOffset: 0,
  selectionType: 'higlight' as 'higlight',
};
...
function addWalkPathToCanvas(
  ...
  var path = new fabric.Path(pathStr, PATH_DEFAULT_OPTIONS);
...
function selectMarker(
  ...
  if ( selectionType === 'pin' ) {
    ...
    console.warn('Only support pin selection type');
    addPinSelector(canvas$, marker, opts);
  }
}
...
function addPinSelector(
  ...
  const lastPin$ =
    getFromCanvasCache(canvas$, 'PIN_MARKER');

  if (lastPin$) {
   canvas$.remove(lastPin$);
  }

  const newPin$ = addPinToCanvas(
    canvas$,
    marker,
    opts
  );
