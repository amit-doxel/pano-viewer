...
const useBlueprint = (props: UseBlueprintProps): void => {
  ...
  useEffect(() => {
    ...
    fabric.Image.fromURL(
      ...
      function (img$: fabric.Image) {
        ...
        const isImgFatched = img$.get('width') !== 0 && img$.get('height') !== 0;

        if (!isImgFatched) {
          console.error('Image.fromURL: Could not fetch blueprint image. Check network tab');
        }
  ...
  useEffect(() => {
    ...
    const localZoom = radiusRatioToViewport * Math.min(canvas$.width!, canvas$.height!) / markerRadius;
...
function deriveMarkerRadius(
  img$: fabric.Image
) {
  return Math.min(img$.getScaledWidth() || 0, img$.getScaledHeight() || 0 ) * 0.01;
}

function derivePathWidth(
  img$: fabric.Image
) {
  return Math.min(img$.getScaledWidth() || 0, img$.getScaledHeight() || 0 ) * 0.003;
}
